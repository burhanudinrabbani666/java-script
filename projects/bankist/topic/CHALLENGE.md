...
const huskyWeight = breeds.find(
  (breed1) => breed1.breed == "Husky"
).averageWeight;

// 2.
// const dogBothActivities = breeds.find((breedData) =>
//   breedData.activities.find((act) => act == "running" && "fetch")
// );
...
const swimmingAdjacent = [
  ...new Set([
    ...breeds
      .filter((breed) => breed.activities.find((act) => act === "swimming"))
      .flatMap((breed) => breed.activities.filter((act) => act !== "swimming")),
  ]),
];

// 6.
const averageWeightarr = breeds
...
console.log(
  breeds
    ...
    .some((act, id, arr) => arr.length >= 3)
);

// 8.
const fetchWeight = breeds
  .filter((act) => act.activities.includes("fetch"))
  .map((breed) => breed.averageWeight);

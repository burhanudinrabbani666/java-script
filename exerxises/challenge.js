...
function printGoals(namePlayer) {
  ...
  for (let index = 0; index < game.scored.length; index++) {
    game.scored[index] == namePlayer && scored++;
  }
  console.log(`${namePlayer} ${scored}`);
}
...
for (const value of avg) oddAvg += value;
oddAvg /= avg.length;
console.log(oddAvg);
...
for (const event of gameEvents.values()) events.push(event);
